...
const PaymentPage: React.FC = () => {
  ...
  const [paymentProcessError, setPaymentProcessError] =
  ...
  const [convertResult, setConvertResult] =
    React.useState<CurrencyConvertResult>();
  ...
  const getCryptoCurrencyConvert = async (params: {
  ...
  const getSupportedMethods = async () => {
    ...
    if (payment.paymentMethod) {
      ...
      try {
        await getCryptoCurrencyConvert({
          cryptoCurrency: payment.paymentMethod.ticker,
        });
    ...
    try {
      const supportedMethods = await paymentProvider.getSupportedMethods(
        payment.id,
      );
  ...
  React.useEffect(() => {
    ...
    if (payment.paymentInfo?.status === "failed") {
      ...
      payment.isLocked &&
      (payment.paymentInfo?.status === "success" ||
  ...
  const onSelectPaymentMethod = async (cryptoCurrency: string) => {
    ...
    const selectedMethod = availableMethods.find(
      (availableMethod) => availableMethod.ticker === cryptoCurrency,
    );
  ...
  const checkCustomer = async (
    e: React.FocusEvent<string, Element>,
    email: string,
  ) => {
  ...
  const genDropDownList = () => {
    ...
    if (!payment?.paymentMethod) {
      resList.push({
        value: "empty value",
        key: "emptyValue",
        displayName: "Select crypto currency",
      });
    }
          ...
          <>
            ...
            <div className="flex relative justify-center mb-7 sm:hidden">
              <QRCodeSVG
                size={180}
                level={"H"}
                value={payment.paymentInfo.recipientAddress}
              />
              <Icon
            ...
            />
            <span className="block mx-auto text-sm mb-7 font-medium text-center text-card-desc sm:hidden">
              Please ensure that you send the correct amount as specified. Make
              sure to include any fees in the total amount to ensure we receive
              the full payment required for your order.
            </span>
          </>
        )}
